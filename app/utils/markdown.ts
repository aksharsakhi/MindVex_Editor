  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__mindvexThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__mindvexArtifact__', '__mindvexThought__', '__mindvexQuickAction', '__mindvexSelectedElement__'],

      // ['className', '__mindvexThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__mindvexArtifact__', '__mindvexThought__', '__mindvexQuickAction'],
    ],
  },
  strip: [],
};
