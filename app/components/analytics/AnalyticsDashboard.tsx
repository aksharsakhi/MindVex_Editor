...
function churnColour(rate: number): string {
    if (rate > 25) return '#f38ba8';   // red
    if (rate > 10) return '#fab387';   // amber
    return '#a6e3a1';                  // green
}
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.header}>
                ...
                <button
                    style={{ ...styles.btn, opacity: mining ? 0.6 : 1 }}
                    onClick={handleMine}
                    disabled={mining}
                >
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart
                                ...
                                onClick={(d: any) => {
                                    ...
                                    if (payload?.fullPath) setSelectedFile(payload.fullPath);
                                }}
                            >
                    ...
                    <div style={styles.panel}>
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={reworkTrend} margin={{ left: 10, right: 20 }}>
                                ...
                                <CartesianGrid strokeDasharray="3 3" stroke="#313244" />
                                <XAxis dataKey="weekStart" stroke="#6c7086" tick={{ fontSize: 10 }}
                                    tickFormatter={d => d.slice(5)} />
                                <YAxis stroke="#6c7086" />
                                <Tooltip contentStyle={tooltipStyle} />
                                <Area type="monotone" dataKey="commits" stroke="#89b4fa"
                                    fill="url(#churnGrad)" strokeWidth={2} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    {selectedFile && trend.length > 0 && (
                        <div style={{ ...styles.panel, gridColumn: '1 / -1' }}>
                            <h3 style={styles.panelTitle}>
                                🔍 {selectedFile.split('/').pop()} — Weekly Churn
                            </h3>
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                                <LineChart data={trend} margin={{ left: 10, right: 20 }}>
                                    ...
                                    <Tooltip contentStyle={tooltipStyle}
                                        formatter={(v: number | undefined) => `${(v ?? 0).toFixed(1)}%`} />
                                    <Line type="monotone" dataKey="churnRate" stroke="#f38ba8"
