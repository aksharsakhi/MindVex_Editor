...
function churnDot(rate: number): { emoji: string; color: string } {
  if (rate > 25) {
    return { emoji: '🔴', color: '#f38ba8' };
  }

  if (rate > 10) {
    return { emoji: '🟡', color: '#fab387' };
  }
...
export function EvolutionaryBlame({ repoUrl, filePath, hotspot }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [blame, fileTrend] = await Promise.all([getBlame(repoUrl, filePath), getFileTrend(repoUrl, filePath, 12)]);
      setBlameLines(blame);
      setTrend(fileTrend);
    } catch (e) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.summaryPanel}>
        <button style={styles.summaryToggle} onClick={() => setPanelOpen((p) => !p)}>
          <span>{dot.emoji}</span>
          <span style={styles.summaryTitle}>
            {filePath.split('/').pop()} — Avg Churn: {avgChurnRate.toFixed(1)}% &nbsp;·&nbsp; {totalCommits} commits
            (12w)
          </span>
          {/* Sparkline */}
          {trend.length > 0 && (
            <div style={styles.sparkline}>
              <ResponsiveContainer width={120} height={32}>
                <LineChart data={trend}>
                  <Line type="monotone" dataKey="churnRate" stroke={dot.color} strokeWidth={1.5} dot={false} />
                  <Tooltip
        ...
        {panelOpen && (
          <div style={styles.summaryBody}>
            ...
            {!aiLoading && aiSummary && <pre style={styles.aiText}>{aiSummary}</pre>}
            {!aiLoading && !aiSummary && (
        ...
        <div ref={listRef} style={styles.blameList}>
          {blameLines.map((line) => (
            <div key={line.lineNumber} style={styles.blameLine}>
              ...
              <span title={`Avg churn: ${avgChurnRate.toFixed(1)}%`} style={{ fontSize: 10, marginRight: 8 }}>
                {dot.emoji}
              </span>
