 ...
 */
export async function uploadScipIndex(repoUrl: string, scipBinary: Blob): Promise<{ jobId: number }> {
  const token = localStorage.getItem('auth_token');
  const formData = new FormData();
  formData.append('file', scipBinary, 'index.scip');

  const res = await fetch(`${BASE_URL}/api/scip/upload?repoUrl=${encodeURIComponent(repoUrl)}`, {
    method: 'POST',
